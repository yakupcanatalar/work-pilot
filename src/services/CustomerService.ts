...
class CustomerService {
    ...
    async getAllCustomers(): Promise<CustomerDTO[]> {
        try {
            ...
            const data = response.data as { content: any }; // Explicitly type response.data
            const customers = Array.isArray(data.content) ? data.content : data.content.customers; // Adjust this line based on your API response structure
    ...
    async updateCustomer(customerId: string, customerData: CustomerDTO){
        try {
